  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      {page === 'Dashboard' && <>
        ...
        <section className="panel"><PanelHead title="Recent inspections" action="View all" onClick={()=>setPage('History')}/>{history.slice(0,6).map(x=><Row key={x.id} item={x} onClick={()=>openInspection(x)}/>)}{!history.length&&<Empty text="No inspections yet. Start with a product image."/>}</section>
      </>}
